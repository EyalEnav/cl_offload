...
void read_image(char *file, struct bmp_header *bmp, struct dib_header *dib, uint8_t **data, uint8_t **palete)
{
    ...
    if(dib->image_size == 0 && (dib->width != 0 || dib->height != 0)) {
        dib->image_size = dib->width * dib->height;
    }

    printf("Image size: %d\n", dib->image_size);
    printf("Image width: %d\n", dib->width);
    printf("Image height: %d\n", dib->height);
    printf("Image horizontal: %d\n", dib->horizontal);
    printf("Image vertical: %d\n", dib->vertical);
    printf("Image bit per pixel: %d\n", dib->bpp);  
    assert(dib->bpp == 8);
...
int main(int argc, char *argv[])
{
    ...
    gpu = 0;
    if(clGetDeviceIDs(platform, gpu ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU, 1, &device_id, NULL) != CL_SUCCESS)
    ...
    //
    cl_int error1;
    input = clCreateBuffer(context,  CL_MEM_READ_ONLY,  dib.image_size, NULL, &error1);
    output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, dib.image_size, NULL, &error1);
    coefs_ver = clCreateBuffer(context,  CL_MEM_READ_ONLY,  COEFS_SIZE, NULL, &error1);
    coefs_hor = clCreateBuffer(context,  CL_MEM_READ_ONLY,  COEFS_SIZE, NULL, &error1);
