...
void DCT8x8CPU(float *dst, float *src, uint stride, uint imageH, uint imageW){

    uint i, j, k;
    for (i = 0; i + BLOCK_SIZE - 1 < imageH; i += BLOCK_SIZE){
...
int main(int argc, char argv[])
{
    ...
    DCT8x8CPU(output, input, stride, img_h, img_w);
